...
- 초당 요청 제한의 정확한 수치는 문서화되어 있지 않다. 실제 라이브 테스트(2026-09-17)로는 배치 크기 8·지연 200ms(초당 약 40건)가 반복적으로 거부당했고, 배치 크기 5·지연 300ms(초당 약 16.7건)는 2006년~현재 전체 기간(약 249개월) 조회를 여러 구에 걸쳐 반복해도 안정적으로 통과했다. 이 값을 더 올리기 전에 반드시 서로 다른 구로 최소 2~3회 반복 테스트해 재현성을 확인한다.
